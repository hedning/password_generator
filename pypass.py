...
parser.add_argument('site', type=str, nargs=1, help='The site you want to the password for')
parser.add_argument('--generate', '-g', dest='generate', action='store_true',
		help='will generate a new password, ie. a new salt')
parser.add_argument('--length', '-l', dest='length', type=int, nargs=1, default=-1, help='the length of the hash')
parser.add_argument('--store', '-s', dest='store', action='store_true',
		help='will store the hash, so next time you call prog with site it will not prompt for the master password')
parser.add_argument('--clean', dest='clean', action='store_true',
		help='Cleans out hashes not in use')
parser.add_argument('--undo', '-u', dest='undo', action='store_true',
		help='Restores the recently used hash for the site.')
...
def get_salt():
	...
	index, info = 0, []
	for index, v in enumerate(salts):
		v = v.split(':')
		if v[0] == site:
			info = v
			break
...
print bcrypt.hashpw(password+site, info[1]).replace(info[1], '')[:length]
